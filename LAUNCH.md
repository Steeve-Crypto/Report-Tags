...
- PostHog project API key: optional. Use `Space Report: Set PostHog API Key` so the key is stored in VS Code SecretStorage, not committed settings.
...
```bash
code --install-extension space-report-0.2.0.vsix
```

Manual smoke test:

1. Run `Space Report: Play Test Sound`.
2. Open a Git workspace.
3. Run `Space Report: Install Git Hooks`.
4. Run `git commit --allow-empty -m "test Space Report"` in the integrated terminal.
...
7. Optional: run `Space Report: Set PostHog API Key`, enable `git-sound-report.telemetry.enabled`, and confirm events arrive in PostHog.
...
4. Upload `space-report-0.2.0.vsix` in Marketplace publisher management.
...
3. Attach `space-report-0.2.0.vsix`.
...
> I launched Space Report, a VS Code extension that plays a quick celebratory sound when Git commands succeed. Free core, adaptive local sounds, optional analytics, and sponsor support.
...
> Space Report now chooses different success sounds based on local Git metadata: fixes, tests, risky changes, deploys, and major releases all feel different.
