...
- PostHog project token: optional. Use `Space Report: Save PostHog Token` so the token is stored in VS Code SecretStorage, not committed settings.
...
7. Optional: run `Space Report: Save PostHog Token`, enable `git-sound-report.telemetry.enabled`, and confirm events arrive in PostHog.
